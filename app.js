...
function getAnimeEntries() {
  return Object.values(userData).filter(isAnimeEntry);
}
...
function formatEpisodeLabel(entry) {
  return `Episode ${entry.episodesWatched || 0} of ${entry.episodes || "?"}`;
}
    ...
    <header class="topnav">
        ...
        <div class="nav-actions">
          <div class="nav-search-inline ${uiState.navSearchOpen ? "is-open" : ""}">
            ...
            <input
              ...
              value="${escapeHtml(uiState.search.query)}"
            >
          </div>
          <button type="button" class="icon-button" data-action="toggle-nav-search" aria-label="Open search">
            <span class="icon-search" aria-hidden="true"></span>
          </button>
          ...
          <button type="button" class="nav-button" data-action="toggle-theme">
            ${uiState.theme === "dark" ? "Light" : "Dark"}
          </button>
        </div>

        <button type="button" class="icon-button topnav__hamburger" data-action="toggle-menu" aria-label="Open menu">
      ...
      <div class="nav-mobile-panel ${uiState.navMenuOpen ? "is-open" : ""}">
        <div class="nav-mobile-panel__card">
          <div class="nav-mobile-panel__tabs">
            ${tabs}
          </div>
          <div class="nav-mobile-panel__actions">
            ...
            <button type="button" class="nav-button" data-action="toggle-theme">
              Switch to ${uiState.theme === "dark" ? "Light" : "Dark"}
            </button>
          </div>
        </div>
      </div>
...
function renderContinueCard(entry) {
  ...
  return `
    <button type="button" class="continue-card" data-action="open-watch" data-id="${entry.id}">
      <div class="continue-card__bg">
        ${image ? `<img src="${escapeHtml(image)}" alt="${escapeHtml(getDisplayTitle(entry))}">` : ""}
      </div>
      <div class="continue-card__content">
        <div class="continue-card__poster">
          ${poster ? `<img src="${escapeHtml(poster)}" alt="${escapeHtml(getDisplayTitle(entry))}">` : ""}
        </div>
      ...
      <div class="continue-card__bar">
        <span style="width:${getProgressPercent(entry)}%"></span>
      </div>
    </button>
  `;
}
...
function renderPosterCard(entry, options = {}) {
  ...
  return `
    <button
      type="button"
      class="poster-card ${context === "grid" ? "poster-card--grid" : ""}"
      data-action="${action}"
      data-id="${entry.id}"
    >
...
function renderMediaRowSection({
  sectionId,
  title,
  subtitle,
  entries,
...
function renderStatsStrip() {
  ...
  return `
    <section class="surface-strip stats-strip">
      <div class="stat-chip">
        <div class="stat-chip__label">Total in library</div>
        <div class="stat-chip__value">${stats.total}</div>
      </div>
      <div class="stat-chip">
        <div class="stat-chip__label">Currently watching</div>
        <div class="stat-chip__value">${stats.watching}</div>
      </div>
      <div class="stat-chip">
        <div class="stat-chip__label">Completed</div>
        <div class="stat-chip__value">${stats.completed}</div>
      </div>
      <div class="stat-chip">
        <div class="stat-chip__label">Episodes watched</div>
        <div class="stat-chip__value">${stats.episodesWatched}</div>
      </div>
    </section>
  `;
}
...
function getFilteredLibraryEntries() {
  ...
  const byFilter =
    uiState.library.filter === "all"
      ? allEntries
      : allEntries.filter((entry) => entry.status === uiState.library.filter);
  ...
  return `
    <div class="page page--library">
      <div class="page-hero">
        ...
        <div class="page-subtitle">Everything saved locally in your browser, ready to browse offline-first.</div>
      </div>
      ...
      .map(([value, label]) => {
        ...
        return `
                  <button
                    type="button"
                    class="chip ${active}"
                    data-action="set-library-filter"
                    data-filter="${value}"
                  >
                    ${label}
                  </button>
      ...
      .join("")}
        ...
        <div class="toolbar-grid">
          <input
            ...
            value="${escapeHtml(uiState.library.query)}"
          >
...
function adaptAniListMedia(media) {
  return {
    id: Number(media.id),
    title: {
      romaji: media.title && media.title.romaji ? media.title.romaji : "Untitled",
      english: media.title && media.title.english ? media.title.english : "",
    },
    coverImage: {
      large: media.coverImage && media.coverImage.large ? media.coverImage.large : "",
    },
...
async function loadBrowse(mode, genre = uiState.browse.genre) {
  ...
  let title = "This Season";
  let subtitle = `${season.charAt(0)}${season.slice(1).toLowerCase()} releases, sorted by popularity.`;

  if (mode === "top") {
    variables = { page: 1, perPage: 30, sort: ["SCORE_DESC"] };
    title = "Top Rated";
    subtitle = "High scoring anime from AniList.";
  } else if (mode === "popular") {
    variables = { page: 1, perPage: 30, sort: ["POPULARITY_DESC"] };
    title = "Most Popular";
    subtitle = "Heavy hitters with the biggest audiences.";
  } else if (mode === "genre") {
    variables = { page: 1, perPage: 30, sort: ["SCORE_DESC"], genre };
    title = `${genre} Highlights`;
    subtitle = `Top rated picks in ${genre}.`;
  } else {
    variables = { page: 1, perPage: 30, sort: ["POPULARITY_DESC"], season, seasonYear };
  }

  try {
    const data = await fetchAniList(BROWSE_QUERY, variables);
    if (requestId !== uiState.browse.requestId) {
      return;
    }

    uiState.browse.results = data.Page.media.map(adaptAniListMedia);
    uiState.browse.title = title;
    uiState.browse.subtitle = subtitle;
...
function renderDiscoverCard(media, source) {
  ...
  const meta = [
    media.averageScore ? `Score ${media.averageScore}` : "",
    media.episodes ? formatCount(media.episodes, "episode") : "Episode total unknown",
  ]
    .filter(Boolean)
    .join(" • ");
  ...
  return `
    <article class="discover-card">
      <div class="discover-card__media">
        ${media.coverImage.large ? `<img src="${escapeHtml(media.coverImage.large)}" alt="${escapeHtml(title)}">` : ""}
      </div>
      ...
      : `
                <button
                  type="button"
                  class="action-button"
                  data-action="open-status-picker"
                  data-source="${source}"
                  data-id="${media.id}"
                >
                  Add to Library
...
function renderBrowse() {
  return `
    <div class="page page--browse">
      <div class="page-hero">
        <div class="page-title">Browse AniList</div>
        <div class="page-subtitle">Seasonal picks, genres, top rated favorites, and audience hits.</div>
      </div>
      ...
      .join("")}
        ...
        <div class="status-line">
          ${escapeHtml(uiState.browse.title)}${uiState.browse.subtitle ? ` - ${escapeHtml(uiState.browse.subtitle)}` : ""}
        </div>
      </section>
...
async function runAniListSearch(query) {
  ...
  try {
    ...
    if (requestId !== uiState.search.requestId) {
      return;
    }
    ...
    uiState.search.loading = false;
    uiState.search.error = error.message;
    queueRender();
  ...
  return `
    <div class="page page--search">
      <section class="search-hero">
        ...
        <input
          id="searchPageInput"
          class="input search-hero__input"
          type="search"
      ...
      <div class="search-layout">
        <section class="section">
          <div class="section__head">
            <div class="section__copy">
              <div class="section__title">Search in My Library</div>
              <div class="section__sub">Local matches update instantly as you type.</div>
            </div>
      ...
      : renderEmptyState(
        ...
        <section class="section">
          <div class="section__head">
            <div class="section__copy">
              <div class="section__title">AniList Results</div>
              <div class="section__sub">Add fresh results directly into AniVault.</div>
            </div>
          </div>
...
function updateStatus(id, status) {
  const entry = getEntry(id);
  if (!entry || !STATUS_OPTIONS.includes(status)) {
    return;
  }
...
async function openWatchView(id) {
  ...
  currentWatchId = id;
  uiState.watch.sidebarCollapsed = false;
...
function switchLanguage(id, language) {
  ...
  entry.language = language === "dub" ? "dub" : "sub";
  saveData();
  uiState.watch.streamLoaded = false;
  renderApp();
}
...
function markEpisodeWatched(id, episode, options = {}) {
  ...
  entry.episodesWatched = Math.max(entry.episodesWatched || 0, targetEpisode);
...
function handlePlaybackEnded() {
  ...
  if (uiState.watch.lastEndedKey === endedKey) {
    return;
  }
...
function renderWatchView() {
  const entry = getEntry(currentWatchId);
  if (!entry) {
    return renderEmptyState("!", "Title missing", "This anime is no longer in your library.");
  }
      ...
      : `
                  <div class="watch-player__fallback">
                    <div class="watch-player__fallback-card">
                      ...
                      <div class="muted">
                        Come back and mark episodes watched manually using the list on the left.
                      </div>
                      <a class="action-button" href="${escapeHtml(fallbackUrl)}" target="_blank" rel="noopener">
                        Search on HiAnime -&gt;
                      </a>
                    </div>
                  </div>
...
function addToLibrary(anilistData, status, options = {}) {
  ...
  const next = normalizeEntry({
    ...(existing || {}),
    id: media.id,
    title: media.title.romaji || (existing && existing.title) || "Untitled",
    titleEnglish: media.title.english || (existing && existing.titleEnglish) || "",
    cover: media.coverImage.large || (existing && existing.cover) || "",
    banner: media.bannerImage || (existing && existing.banner) || "",
    episodes: media.episodes || (existing && existing.episodes) || 0,
    status: status || (existing && existing.status) || "untracked",
    episodesWatched: (existing && existing.episodesWatched) || 0,
    language: (existing && existing.language) || "sub",
    rating: (existing && existing.rating) || 0,
    dateAdded: (existing && existing.dateAdded) || Date.now(),
    lastWatched: (existing && existing.lastWatched) || 0,
    completedAt: (existing && existing.completedAt) || 0,
    notes: (existing && existing.notes) || "",
    genres: media.genres || (existing && existing.genres) || [],
    year: media.seasonYear || (existing && existing.year) || 0,
    anilistId: media.id,
    sessionLog: (existing && existing.sessionLog) || [],
    averageScore: media.averageScore || (existing && existing.averageScore) || 0,
  });
...
function openDetailOverlay(id) {
  ...
  uiState.overlay = { type: "detail", id };
  renderApp();
}
...
function renderOverlay() {
  ...
  if (uiState.overlay.type === "status-picker") {
    const media = getAniListResultBySource(uiState.overlay.source, uiState.overlay.id);
    if (!media) {
      return "";
    }
    ...
    return `
      <div class="overlay" data-action="close-overlay">
        <div class="overlay-card" role="dialog" aria-modal="true" aria-label="Add to library" data-overlay-card>
          <div class="overlay-card__meta">
            <div class="overlay-card__title">Add ${escapeHtml(title)}</div>
            <div class="muted">Choose how this title should enter your private library.</div>
          </div>
          <div class="status-picker">
            <button type="button" class="action-button" data-action="picker-status" data-source="${uiState.overlay.source}" data-id="${media.id}" data-status="watching">Start Watching</button>
            <button type="button" class="secondary-button" data-action="picker-status" data-source="${uiState.overlay.source}" data-id="${media.id}" data-status="queued">Add to Queue</button>
            <button type="button" class="secondary-button" data-action="picker-status" data-source="${uiState.overlay.source}" data-id="${media.id}" data-status="plan-to-watch">Save for Later</button>
            <button type="button" class="nav-button" data-action="close-overlay">Cancel</button>
          </div>
        </div>
      </div>
    `;
  ...
  if (uiState.overlay.type === "detail") {
    const entry = getEntry(uiState.overlay.id);
    if (!entry) {
      return "";
    }
    ...
    return `
      <div class="overlay" data-action="close-overlay">
        <div class="overlay-card" role="dialog" aria-modal="true" aria-label="Anime details" data-overlay-card>
          <div class="overlay-card__hero">
            <div class="overlay-card__cover">
              ${entry.cover ? `<img src="${escapeHtml(entry.cover)}" alt="${escapeHtml(getDisplayTitle(entry))}">` : ""}
            </div>
            <div class="overlay-card__meta">
              <div class="overlay-card__title">${escapeHtml(getDisplayTitle(entry))}</div>
              <div class="muted">${escapeHtml(formatCount(entry.episodes || 0, "episode"))}${entry.year ? ` • ${entry.year}` : ""}</div>
              <div class="watch-meta__row">
                <span class="${getStatusClass(entry.status)}">${escapeHtml(STATUS_LABELS[entry.status])}</span>
                ${entry.averageScore ? `<span class="watch-badge">AniList ${entry.averageScore}</span>` : ""}
              </div>
              <select class="select" data-status-select="${entry.id}">
                ${STATUS_OPTIONS.map((status) => `<option value="${status}" ${entry.status === status ? "selected" : ""}>${STATUS_LABELS[status]}</option>`).join("")}
              </select>
            </div>
          </div>
...
function showToast(message, type = "info") {
  ...
  toast.innerHTML = `
    <div class="toast__title">${escapeHtml(TOAST_TITLES[type] || TOAST_TITLES.info)}</div>
...
function exportData() {
  ...
  anchor.href = url;
  anchor.download = `anivault-backup-${stamp}.json`;
...
function importData(event) {
  ...
  reader.onload = () => {
    try {
      ...
      saveData();
      renderApp();
      showToast(`Imported ${count} ${count === 1 ? "title" : "titles"}.`, "success");
    } catch (error) {
...
function renderApp() {
  app.innerHTML = `
    <div class="app-shell">
      ...
      ${renderOverlay()}
      ${renderMobileTabs()}
      <input id="importInput" class="visually-hidden" type="file" accept=".json">
    </div>
  `;
...
function afterRender() {
  document.querySelectorAll("[data-row-track]").forEach((track) => {
    syncScrollButtons(track.id);
    track.addEventListener("scroll", () => syncScrollButtons(track.id), { passive: true });
  });

  if (uiState.navSearchOpen) {
  ...
  if (uiState.focusInputId) {
    ...
    uiState.focusInputId = "";
  }

  if (currentTab === "search" && !currentWatchId) {
    const pageInput = document.getElementById("searchPageInput");
    if (pageInput && document.activeElement !== pageInput && uiState.search.query) {
      pageInput.setSelectionRange(pageInput.value.length, pageInput.value.length);
    }
  }

  if (currentTab === "browse" && !uiState.browse.initialized && !uiState.browse.loading) {
    loadBrowse("seasonal");
  }
...
function setupWatchPlayer() {
  ...
  streamFallbackTimer = window.setTimeout(() => {
    if (!uiState.watch.streamLoaded && currentWatchId) {
      ...
      renderApp();
      showToast("MegaPlay did not load. Manual tracking is still available.", "error");
    }
  }, 7000);
}
...
function syncScrollButtons(trackId) {
  ...
  const prevButton = document.querySelector(`[data-target="${trackId}"][data-dir="prev"]`);
  const nextButton = document.querySelector(`[data-target="${trackId}"][data-dir="next"]`);
...
function handleClick(event) {
  ...
  if (action === "tab") {
    renderTab(actionTarget.dataset.tab);
    return;
  }
  ...
  if (action === "toggle-nav-search") {
    uiState.navSearchOpen = !uiState.navSearchOpen;
    if (uiState.navSearchOpen) {
      currentTab = "search";
    }
    renderApp();
  ...
  if (action === "row-scroll") {
    scrollRow(actionTarget.dataset.target, actionTarget.dataset.dir);
    return;
  }

  if (action === "open-watch") {
    openWatchView(Number(actionTarget.dataset.id));
    return;
  }

  if (action === "open-detail") {
    openDetailOverlay(Number(actionTarget.dataset.id));
    return;
  }
  ...
  if (action === "open-status-picker") {
    openStatusPicker(actionTarget.dataset.source, Number(actionTarget.dataset.id));
    return;
  }
  ...
  if (action === "set-library-filter") {
    uiState.library.filter = actionTarget.dataset.filter;
    renderApp();
    return;
  }

  if (action === "browse-mode") {
    loadBrowse(actionTarget.dataset.mode);
    return;
  }

  if (action === "browse-genre") {
    loadBrowse("genre", actionTarget.dataset.genre);
    return;
  }

  if (action === "watch-back") {
    closeWatchView();
    return;
  }

  if (action === "watch-prev") {
    if (currentWatchId) {
  ...
  if (action === "switch-language") {
    if (currentWatchId) {
      switchLanguage(currentWatchId, actionTarget.dataset.lang);
    }
    return;
  }
  ...
  if (action === "set-episode") {
    if (currentWatchId) {
      switchEpisode(currentWatchId, Number(actionTarget.dataset.ep));
    }
    return;
  }
...
function handleInput(event) {
  ...
  if (event.target.id === "searchPageInput" || event.target.id === "navSearchInput" || event.target.id === "mobileNavSearchInput") {
...
function handleChange(event) {
  if (event.target.id === "librarySortSelect") {
    uiState.library.sort = event.target.value;
    renderApp();
    return;
  }

  if (event.target.id === "importInput") {
    importData(event);
    return;
  }

  if (event.target.dataset.statusSelect) {
    updateStatus(Number(event.target.dataset.statusSelect), event.target.value);
  }
}

function handleFocusOut(event) {
  if (!event.target.dataset.notesField) {
    return;
  }

  const entry = getEntry(Number(event.target.dataset.notesField));
  if (!entry) {
    return;
  }

  entry.notes = event.target.value.trim();
  saveData();
...
function handleMessage(event) {
  if (!event.origin.includes("megaplay.buzz") || !currentWatchId) {
    return;
  }
...
function init() {
  ...
  window.addEventListener("resize", () => {
    document.querySelectorAll("[data-row-track]").forEach((track) => syncScrollButtons(track.id));
  });

  if (uiState.search.query.trim().length >= 2) {
    scheduleAniListSearch(uiState.search.query);
  }
}
